...
class ExamplesPerSecondHook(basic_session_run_hooks.StepCounterHook):
  ...
  def _log_and_record(self, elapsed_steps, elapsed_time, global_step):
    ...
    if self._summary_writer is not None:
      ...
      total_time_summary = Summary(value=[
          Summary.Value(tag='total_time', simple_value=self._total_time)
      ])   
      self._summary_writer.add_summary(total_time_summary, global_step)
    logging.info('elapsed_steps: %d', elapsed_steps)
    logging.info('total_time: %d', self._total_time)
